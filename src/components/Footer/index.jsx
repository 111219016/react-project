import './footer.css';

const Footer = () => {
    return (

        <footer className="bg-black text-[#FFFBF3] py-6 mt-15">
            <div className="container mx-auto px-4 text-center space-y-2 sm:space-y-3 md:space-y-4">
                <p className="text-[16px] sm:text-[16px] md:text-[24px] font-bold">名畫檔案</p>
                <p className="text-[12px] sm:text-[12px] md:text-[16px] ">聯絡我們</p>
                <p className="text-[12px] sm:text-[12px] md:text-[16px] ">電話：07-3710240</p>
                <p className="text-[12px] sm:text-[12px] md:text-[16px] ">814 高雄市仁武區登峰街58號</p>
            </div>
         </footer>
    );
  };
  
  export default Footer;
